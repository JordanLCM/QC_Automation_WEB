...
public class Logout {
	...
	public void verifyLogin() throws InterruptedException {
		...
		if (qctester01010101.isDisplayed()) {
			Actions action = new Actions(baseD.getDriver());
			action.moveToElement(qctester01010101).perform();
			createR.setExtentTest().info("Logged in to userID " + userID);
		}
	}

	String logoutAttribute;

	@Test(dependsOnMethods = { "verifyLogin" })
	public void clickLogoutButton() throws InterruptedException {
		createR.createTest("Click Logout Button");

		Thread.sleep(1000);
		WebElement 登出Button = baseD.getDriver().findElement(By.xpath("//button[contains(text(),'登出')]"));
		logoutAttribute = 登出Button.getText();
		if (登出Button.isDisplayed()) {
			if (logoutAttribute.equals("登出")) {
				登出Button.click();
				Thread.sleep(2000);
				takeSS = new TakeScreenshot(baseD);
				takeSS.takeScreenShot(logoutAttribute + " passed");
				createR.setExtentTest().info("Logged out of userID " + userID).addScreenCaptureFromPath(screenShotPath + logoutAttribute + " passed" + ".png");
			}
		} else {
			softAssert.assertTrue(false);
			createR.setExtentTest().info("Click Login Option is failed");
